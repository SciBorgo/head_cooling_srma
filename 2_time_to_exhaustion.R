...
# Plot
png(file = 'figures/time_to_exhaustion.png', width = 6, height = 3.75, res = 600, units = "in") 
viz_forest(study_table = data.frame('Study' = d$study,
                                    'Subgroup' = d$subgroup),
           x = fit_mods,
...
overall_results %>%
  write.csv(file = 'results/table_time_to_exhaustion.csv',
            row.names = F)
...
final_results %>%
  ...
  cols_align(align = "left") -> sens_table_tte; sens_table_tte

gtsave(sens_table_tte, "sensitivity results/sensitivity_tte.docx")
