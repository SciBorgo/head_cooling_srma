...
# Load data
d <- read_xlsx("data/T site skin.xlsx") %>%
  clean_names() %>%
...
# Plot
png(file = 'figures/skin_temp_at_site.png', width = 7, height = 7, res = 600, units = "in") 
...
overall_results %>%
  write.csv(file = 'results/table_skin_temp_at_site.csv',
            row.names = F)
...
final_results %>%
  ...
  cols_align(align = "left") -> sens_table_skin_target; sens_table_skin_target

gtsave(sens_table_skin_target, "sensitivity results/sensitivity_target_site_skin_temp.docx")
