...
# Load data
d <- read_xlsx("data/T mean skin.xlsx") %>%
  clean_names() %>%
...
# # Fit with robust variance estimation
corr = 0.7
d$vi_z = sqrt(d$vi)
V <- with(d, impute_covariance_matrix(vi = vi_z,
                                      cluster = study,
                                      r = corr))

fit_robust <-
  rma.mv(yi ~ 1 + subgroup,
         V = V,
         random = ~ 1 | study/es_id,
         data = d,
         sparse = T)

conf_int(fit_robust, vcov = 'CR2')
coef_test(fit_robust, vcov = "CR2")

# Check subgroup effect without Gordon
# fit_mods <- rma.mv(yi = yi,
#                    V = vi,
#                    slab = study,
#                    dat = d %>% filter(study != 'Gordon et al., 2023'),
#                    random = ~ 1 | study/es_id,
#                    mods = ~ subgroup,
#                    test = "t",
#                    method = "REML",
...
# Plot
png(file = 'figures/mean_skin_temp.png', width = 7, height = 7, res = 600, units = "in") 
...
overall_results %>%
  write.csv(file = 'results/table_mean_skin_temp.csv',
            row.names = F)
...
final_results %>%
  ...
  cols_align(align = "left") -> sens_table_skin; sens_table_skin

gtsave(sens_table_skin, "sensitivity results/sensitivity_skin_temp.docx")
